        ...
        <form @submit.prevent="handleSubmit" class="w-full max-w-lg">
            <div class="flex flex-wrap -mx-3 mb-6">
                <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                    ...
                    <input v-model="recipe_name" class="appearance-none block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="recipe-name" type="text" placeholder="Chicken Parmesean">
                </div>
                <div class="w-full md:w-1/2 px-3">
                    <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="last-name">
                        Last Name
                    </label>
                    <input class="appearance-none block w-full bg-gray-200 text-gray-700 border-red-500 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="last-name" type="text" placeholder="Doe">
...
<script setup lang="ts">
    ...
    let handleSubmit = async () => {
        if(!recipe_name.value || recipe_name.value === '') {
        ...
        const { data, error } = await supabase
            ...
            .insert([
                // @ts-ignore
                { title: recipe_name.title, slug: 'taco-dip' },
            ])
